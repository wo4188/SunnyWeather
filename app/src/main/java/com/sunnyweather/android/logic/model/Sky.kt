...
//因为彩云天气返回的数据中，天气情况都是一些诸如CLOUDY、WIND之类的天气代码，
//因此需要编写一个额外的转换函数将这些天气代码转换成一个Sky对象。

data class Sky(val info: String, val icon: Int, val bg: Int)

//这里有些图标和背景是一模一样的，凑合着用
...
//这个就是转换函数
fun getSky(skycon: String): Sky {       //skycon即sky condition 天空(气)情况
    return sky[skycon] ?: (sky["CLEAR_DAY"])!!       //使用!!时，会对对象进行非空判断，并且会像java代码一样抛出空指针异常
    //对象A ?: 对象B 表达式。即，只有当对象A值为null时，那么才会返回后面的对象B，否则返回对象A。
}